...
public:
	...
	ExtendedLQR(const Matrix<XDIM, 1>& s, const Matrix<XDIM,1>& cg, const int& num, const double& Deltat, const int& cal_t)
	{
		cal_trace = cal_t;
		dt = Deltat;
		totalTime = 0.0;
	...
	{
		std::ofstream fout("total_value.txt");
		for(int i = 0; i <= T; i++){
			...
			fout<<thisstepTotalCost<<std::endl;
		}
		fout.close();
		return true;
	}
	...
	void showplan(const std::vector<Matrix<XDIM, 1>>& sequencestates)
	{	
		CAL_EmptyGroup(cal_trace);
		for(int i = 0; i < (int)sequencestates.size(); i++){
			CAL_CreateSphere(cal_trace, 0.05, sequencestates[i][0], sequencestates[i][1], sequencestates[i][2]);
		}
		return;
	}
	...
	inline void extendedLQG(std::vector<Matrix<UDIM,XDIM> >& L, std::vector<Matrix<UDIM> >& l)
	{
		...
		for (int iter = 0; iter < maxIter; ++iter) {
			...
			for (int t = 0; t < T; ++t) {
				...
				cf.quadratizeCost(xHat, uHat, t, iter, Pt, Qt, Rt, qvect, rvect, qscalet);
			...
			// backward pass
			cf.quadratizeFinalCost(xHat, S[T], s[T], sscale[T], iter);
			...
			for (int t = T - 1; t != -1; --t) {
				...
				Matrix<XDIM, 1> qvect; Matrix<UDIM, 1> rvect;
				double qscalet = 0;
				cf.quadratizeCost(xHatPrime, uHat, t, iter, Pt, Qt, Rt, qvect, rvect, qscalet);
