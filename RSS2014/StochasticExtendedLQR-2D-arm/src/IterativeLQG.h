#ifndef _ITERATIVELQG
#define _ITERATIVELQG
...
public:
	...
	double lambda; //control the parameter of the obstable avoidance.
	...
	IterativeLQG(const Matrix<XDIM, 1>& s, const Matrix<2,1>& g, const Matrix<XDIM,1>& cg,
				const int& numS,
	...
	inline double computeCost(const std::vector<std::pair<Matrix<XDIM, 1>, Matrix<UDIM, 1>>>& plan)
	{
		...
		cost += cf.cost0(plan[0].first, plan[0].second);
		for(int i = 1; i < (int)plan.size() - 1; i++){
			cost += cf.costt(plan[i].first, plan[i].second);
			//std::cout<<plan[i].first<<std::endl;
			//cost += cf.costt_ov(plan[i].first, plan[i].second, cal_l1, cal_l2, cal_obs);
		}
		//cost += cf.costT(plan[(int)plan.size() - 1].first);
		cost += cf.costcT(plan[(int)plan.size() - 1].first);
		return cost;
	}
	...
	inline void BvalueIteration(const std::vector<std::pair<Matrix<XDIM>, Matrix<UDIM>>>& plan)
	{
		...
		//cf.QuadratizeCostT_real(plan[T].first, plan[T].second, QT, qvecT, qT);
		...
		//std::cout<<"Value iter:: "<<0.5 * tr(~plan[0].first * S * plan[0].first) + tr(~svec * plan[0].first) + sscale<<std::endl;
