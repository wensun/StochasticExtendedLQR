...
template <size_t xDim>
void regularize(Matrix<xDim, xDim>& Q) {
	SymmetricMatrix<xDim> symQ = SymProd(Q, identity<xDim>());
	SymmetricMatrix<xDim> D;
	Matrix<xDim,xDim> V;
	jacobi(symQ, V, D);
	for (size_t i = 0; i < xDim; ++i) {
...
//map: R^6 -> se[3]
inline Matrix<4,4> upper(const Matrix<6>& x)
{
	Matrix<4,4> se(zeros<4,4>());
	Matrix<3> p = x.subMatrix<3,1>(0,0);
	Matrix<3> r = x.subMatrix<3,1>(3,0);

	se.insert<3,3>(0,0, cpMatrix(r));
	se.insert<3,1>(0,3, p);
...
//map: so(3) - > R^3 (r)
inline Matrix<3> down(const Matrix<3,3>& so)
{
	Matrix<3> r(zeros<3,1>());
...
//log SO to so
inline Matrix<3,3> logm(const Matrix<3,3>& R)
{
	double theta = acos((tr(R) - 1.0)/2.0);
	Matrix<3,3> skewSymmW = 1.0/2.0/sin(theta) * (R - ~R);

	return theta * skewSymmW;
}

// Switch between belief vector and matrices
template<int bDim, int xDim>
void unVec(const Matrix<bDim>& b, Matrix<xDim>& x, Matrix<xDim,xDim>& SqrtSigma) {
	x = b.subMatrix<xDim,1>(0,0);
	int idx = xDim;
	for (int j = 0; j < xDim; ++j) {
		for (int i = j; i < xDim; ++i) {
...
template<int xDim, int bDim>
void vec(const Matrix<xDim>& x, const Matrix<xDim,xDim>& SqrtSigma, Matrix<bDim>& b) {
	b.insert(0,0,x);
	int idx = xDim;
	for (int j = 0; j < xDim; ++j) {
		for (int i = j; i < xDim; ++i) {
...
inline SymmetricMatrix<aDim> hessian(const Matrix<aDim>& a, const T& t, double (*f)(const Matrix<aDim>&, const T&), double jStep) {
